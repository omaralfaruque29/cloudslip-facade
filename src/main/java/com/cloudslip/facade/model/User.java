...
public class User extends BaseEntity implements UserDetails {
    ...
    public ObjectId getCompanyId() {
        if(companyId != null) {
            return companyId;
        } else if(userInfo != null && userInfo.getCompany() != null) {
            this.setCompanyId(userInfo.getCompany().getObjectId());
            return userInfo.getCompany().getObjectId();
        }
        return null;
    }
    ...
    @JsonIgnore
    public String getCompanyIdFromUserInfo() {
        if(this.userInfo != null && this.userInfo.getCompany() != null) {
            return this.userInfo.getCompany().getId();
        }
        return "";
    }

    @JsonIgnore
    public String getOrganizationIdFromUserInfo() {
        if(this.userInfo != null && this.userInfo.getOrganization() != null) {
            return this.userInfo.getOrganization().getId();
        }
        return "";
    }

    @JsonIgnore
    public String getTeamIdsFromUserInfo() {
        String teamIdsStr = "";
        if(this.userInfo != null && this.userInfo.getTeams() != null && this.userInfo.getTeams().size() > 0) {
            for (Team team: this.userInfo.getTeams()) {
                teamIdsStr = teamIdsStr.concat(team.getId().concat(","));
            }
    ...
    public String toJsonString() {
        return
            ...
            ", \"companyId\": " + "\"" + getCompanyIdFromUserInfo() + "\"" +
            ", \"organizationId\": " + "\"" + getOrganizationIdFromUserInfo() + "\"" +
            ", \"teamIds\": " + "\"" + getTeamIdsFromUserInfo() + "\"" +
            ", \"userType\": " + "\"" + getUserTypeAsString() + "\"" +
            "}";
    }
}
